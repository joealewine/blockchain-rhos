...
keywords: OpenShift, IBM Blockchain Platform console, IBM, resource requirements, storage, parameters
...
# Deploying {{site.data.keyword.blockchainfull_notm}} Platform v2.1.1 from IBM
{: #deploy-ocp-ibm}

These instructions are provided for IBMers only to access the images.

## Before you begin
{: #deploy-ocp-ibm-before}

In order to gain access to the images you must first submit an entitlement request and generate an API key that can be used for deployment. See this [README](https://github.ibm.com/IBM-Blockchain/ibp-requests/blob/master/ibp-on-openshift/README.md) for more information.
